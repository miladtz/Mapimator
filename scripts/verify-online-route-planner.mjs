...
assert.match(app, /Maritime: Built-in · Approximate/);
assert.match(app, /Air: Built-in · Explicit calculation/);
assert.match(app, /Needs calculation/);
assert.match(app, /Every Route Section must be Ready before Use Route/);
...
assert.match(planner, /section\.status !== 'ready' \|\| section\.pathType === 'custom'/);
assert.match(planner, /canUseRoutePlannerDraft/);
assert.match(planner, /routePlannerSectionsNeedingCalculation/);
